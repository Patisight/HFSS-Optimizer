...
from algorithms import NSGA2, SurrogateAssistedNSGA2
...
def run_optimization(config: dict, algorithm: str = 'surrogate'):
    ...
    try:
        ...
        # 创建可视化器
        plot_interval = config.get('visualization', {}).get('plot_interval', 5)
        visualizer = OptimizationVisualizer(run_dir, config['objectives'], config.get('variables', []), plot_interval)
        
        # 定义回调函数 - 用于迭代过程中更新可视化
        iteration_count = [0]  # 用列表包装以便在闭包中修改
        
        def progress_callback(current, total, params, objectives, phase):
            """迭代回调：更新进度和可视化"""
            iteration_count[0] += 1
            ...
            # 更新可视化
            try:
                visualizer.update(iteration_count[0], params, obj_list)
            except Exception as e:
        ...
        for i, sol in enumerate(pareto_params[:5]):
            print(f"\n  Solution {i+1}:")
            ...
            if isinstance(objectives, list):
                # MOBO 格式：列表 - 需要将 maximize 目标的值取反回来
                print(f"    Objectives:")
