...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QLabel, QComboBox, QSpinBox, QDoubleSpinBox,
    QGroupBox, QFormLayout, QScrollArea, QMessageBox, QSplitter
)
...
class ResultViewer(QMainWindow):
    ...
    def export_selected(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "导出", f"export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            "JSON (*.json)"
        )
        if not path:
            return
