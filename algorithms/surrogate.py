...
class LatinHypercubeSampler:
    ...
    def generate(self, n_samples: int) -> np.ndarray:
        ...
        try:
            ...
            # 回退到随机采样
            return np.random.uniform(self.bounds[:, 0], self.bounds[:, 1], size=(n_samples, self.n_vars))
...
class SurrogateAssistedNSGA2(NSGA2):
    ...
    def run(self, evaluator) -> List[Dict]:
        ...
        for gen in range(self.n_generations):
            ...
            population, objectives, all_results = self._select_next_generation(
                population, objectives, all_results, offspring[0], offspring[1], offspring[2]
            )
    ...
    def get_statistics(self) -> Dict:
        ...
        stats["simulation_savings"] = (
            (self.surrogate_eval_count / max(self.evaluation_count, 1)) * 100 if self.evaluation_count > 0 else 0
        )
        return stats

    def _init_surrogate(self):
        """初始化代理模型"""
        self.surrogate = GPSurrogateModel(len(self.variables), len(self.objectives))
    ...
    def _validate_and_extract(self, front, population, all_results, evaluator) -> List[Dict]:
        ...
        for idx in front:
            ...
            # 如果是代理模型结果，进行真实验证
            if results and any(res.get("is_surrogate", False) for res in results.values() if isinstance(res, dict)):
                logger.debug(f"  Validating solution with real simulation...")
                result = self._evaluate_with_cache(params, evaluator)
                if result is not None:
                    results = result[1]

            param_dict = {v["name"]: round(params[i], 4) for i, v in enumerate(self.variables)}
