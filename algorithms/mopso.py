...
from core.surrogate import GPflowSVSManager, IncrementalSurrogateManager, SurrogateManager
...
class MOPSO(BaseOptimizer):
    ...
    def run(self, evaluator, callback: Callable = None) -> List[Dict]:
        ...
        if self.use_surrogate:
            ...
            if self.dual_line_mode:
                ...
                self.surrogate_manager = IncrementalSurrogateManager(
                    n_objectives=self.n_objectives, min_samples=min_samples, n_features=n_features, gamma=gamma
                )
                ...
                self.surrogate_manager = SurrogateManager(
                    n_objectives=self.n_objectives, model_type="rf", min_samples=min_samples, n_estimators=n_estimators
                )
                logger.info(
                    f"[INFO] Surrogate: RF (n_estimators={n_estimators}, retrain_interval={self.retrain_interval})"
                )
                ...
                self.retrain_interval = model_params.get("retrain_interval", self.retrain_interval)
                self.surrogate_manager = SurrogateManager(
                    n_objectives=self.n_objectives, model_type="gp", min_samples=min_samples
                )
            ...
            if self.loaded_evaluations:
                ...
                for i, obj_config in enumerate(self.objectives):
                    if i < all_objectives.shape[1]:
                        logger.info(
                            f"  {obj_config.get('name')}: min={all_objectives[:, i].min():.2f}, max={all_objectives[:, i].max():.2f}, std={all_objectives[:, i].std():.2f}"
                        )
                ...
                if self.surrogate_manager.surrogate.is_trained:
                    ...
                    logger.info(
                        f"  Predicted: S11={test_y_pred.flatten()[0]:.2f}, PG={-test_y_pred.flatten()[1]:.2f} dB"
                    )
        ...
        use_surrogate_for_init = (
            self.use_surrogate
            and self.surrogate_manager
            and self.surrogate_manager.surrogate.is_trained
            and len(self.loaded_evaluations) >= self.surrogate_manager.min_samples_to_train
        )
        ...
        if self.stop_when_goal_met:
            ...
            if len(archive_objectives) > 0:
                ...
                logger.info(
                    f"[INFO] Goals check: {goals_count} solutions meet goals (threshold: {self.n_solutions_to_stop})"
                )
            ...
            for i in range(self.population_size):
                ...
                if self.callback:
                    if not self.is_penalty_value(y):
                        ...
                        is_surrogate = not is_real
                        self.callback(
                            gen, self.n_generations, self.particles[i], y, "iteration", surrogate_preds, is_surrogate
                        )
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if len(params) != self.n_variables:
                            logger.warning(
                                f"[WARN] Skip record: param count mismatch ({len(params)} vs {self.n_variables})"
                            )
                            continue

                        if len(objectives) != self.n_objectives:
                            logger.warning(
                                f"[WARN] Skip record: objective count mismatch ({len(objectives)} vs {self.n_objectives})"
                            )
                            continue
                        ...
                        # 存储加载的数据
                        self.loaded_evaluations.append({"params": params, "objectives": objectives})
    ...
    def _initialize_particles(self, n_vars: int):
        ...
        if self.loaded_evaluations and len(self.loaded_evaluations) > 0:
            ...
            for i in range(n_from_history):
                ...
                perturbation = np.array(
                    [np.random.uniform(-0.05, 0.05) * (self.bounds[j, 1] - self.bounds[j, 0]) for j in range(n_vars)]
                )
                particle = np.clip(particle + perturbation, self.bounds[:, 0], self.bounds[:, 1])
                self.particles.append(particle)

                velocity = np.array(
                    [np.random.uniform(-0.1, 0.1) * (self.bounds[j, 1] - self.bounds[j, 0]) for j in range(n_vars)]
                )
        ...
        for i in range(n_from_history, self.population_size):
            particle = np.array([np.random.uniform(self.bounds[j, 0], self.bounds[j, 1]) for j in range(n_vars)])
            self.particles.append(particle)
    ...
    def _evaluate(self, x: np.ndarray, evaluator, force_real: bool = False) -> Tuple[np.ndarray, bool]:
        ...
        if self.use_surrogate and self.surrogate_manager:
            if self.surrogate_manager.min_samples_to_train < 5:
                logger.warning(
                    f"[WARN] surrogate_min_samples={self.surrogate_manager.min_samples_to_train} is too low, raising to 5"
                )
        ...
        if self.use_surrogate and self.surrogate_manager and not force_real:
            ...
            if self.real_evaluation_count >= min_samples and self.surrogate_manager.surrogate.is_trained:
                ...
                # 调试输出：打印预测值和不确定性
                logger.info(
                    f"  [Surrogate Debug] y_pred={y_pred}, y_std={y_std}, uncertainty={normalized_uncertainty:.3f}"
                )
                ...
                if normalized_uncertainty < self.surrogate_threshold:
                    ...
                    logger.info(
                        f"  [Surrogate] Eval #{self.evaluation_count} (uncertainty: {normalized_uncertainty:.3f} < {self.surrogate_threshold}, prediction)"
                    )
                    return y_pred, False
            ...
            if self.dual_line_mode:
                ...
                if self.retrain_interval > 0 and self.surrogate_type in ["gp", "rf", "gpflow_svgp"]:
                    ...
                    if samples_since_retrain >= self.retrain_interval:
                        logger.info(
                            f"  [Retraining] Full retrain triggered (interval={self.retrain_interval}, new samples={samples_since_retrain})"
                        )
                        self.surrogate_manager.retrain_all()
    ...
    def _update_archive(self, x: np.ndarray, y: np.ndarray, is_predicted: bool = False):
        ...
        # 添加新解
        self.archive.append({"parameters": x.tolist(), "objectives": y.tolist(), "is_predicted": is_predicted})
