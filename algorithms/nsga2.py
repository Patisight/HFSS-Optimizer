...
class NSGA2(BaseOptimizer):
    ...
    def run(self, evaluator) -> List[Dict]:
        ...
        for gen in range(self.n_generations):
            ...
            population, objectives, all_results = self._select_next_generation(
                population, objectives, all_results, offspring, offspring_objectives, offspring_results
            )
    ...
    def _initialize_population(self) -> List[np.ndarray]:
        ...
        for _ in range(self.population_size):
            individual = np.array([np.random.uniform(bounds[i, 0], bounds[i, 1]) for i in range(len(self.variables))])
    ...
    def _extract_pareto_params(self, front: List[int], population, all_results) -> List[Dict]:
        ...
        for idx in front:
            ...
            param_dict = {v["name"]: round(float(params[i]), 4) for i, v in enumerate(self.variables)}
