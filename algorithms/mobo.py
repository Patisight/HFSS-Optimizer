    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        try:
            ...
            self.model = GaussianProcessRegressor(
                kernel=kernel, n_restarts_optimizer=10, normalize_y=self.normalize_y, random_state=42
            )
...
class MultiObjectiveBayesianOptimizer(BaseOptimizer):
    ...
    def run(self, evaluator, callback: Callable = None) -> List[Dict]:
        ...
        if self.stop_when_goal_met:
            goals_count = self.count_objectives_meeting_goals_from_arrays(
                [self.y_observed[i] for i in range(len(self.y_observed))]
            )
            logger.info(
