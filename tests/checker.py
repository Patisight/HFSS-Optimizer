...
"""
    ...
    def _check_variable_bounds(self):
        ...
        for var in variables:
            ...
            if var_errors:
                failed_bounds.append({"name": name, "errors": var_errors})
            else:
                passed_count += 1
                print(f"  {name}: 边界测试通过")

        if failed_bounds:
            self.results["warnings"].append(f"{len(failed_bounds)}/{len(variables)} 个变量边界会导致模型错误")
    ...
    def _check_random_samples(self, n_samples: int = 10):
        ...
        for i in range(n_samples):
            ...
            if set_errors:
                failed_samples.append(
                    {"sample": i + 1, "params": param_display, "errors": set_errors, "type": "设置失败"}
                )
        ...
        if fail_count > 0:
            ...
            if fail_rate >= 50:
                self.results["errors"].append(f"随机采样失败率过高: {fail_rate:.0f}% ({fail_count}/{n_samples})")
            elif fail_count > 0:
                self.results["warnings"].append(f"随机采样失败: {fail_count}/{n_samples} ({fail_rate:.0f}%)")
                    ...
                    for sweep_name in sweeps:
                        try:
                            ...
                            if hasattr(sweep, "props") and hasattr(sweep.props, "get"):
                                ...
                                if not save_rad:
                                    self.results["warnings"].append(f"Sweep '{sweep_name}' 未保存辐射场")
                                else:
                                    self.results["passed"].append(f"Sweep '{sweep_name}' 已保存辐射场")
                        except:
                            pass

                self.results["details"]["setup"] = {"frequency": freq, "far_field_index": ff_index, "sweeps": sweeps}
                    ...
                    if self.hfss and self.hfss._connected:
                        try:
                            ...
                            if setup_freq and abs(setup_freq - freq) > 0.1:
                                ...
                                self.results["details"][f"{name}_freq_mismatch"] = {"target": freq, "setup": setup_freq}
                            else:
    ...
    def get_report_text(self) -> str:
        ...
        # 统计
        lines.append(
            f"通过: {summary.get('passed', 0)} | 警告: {summary.get('warnings', 0)} | 错误: {summary.get('errors', 0)}"
        )
        lines.append("")
