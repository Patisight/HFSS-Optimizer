...
class TrainerProcess:
    ...
    def initialize(self):
        ...
        try:
            if self.model_type == "gpflow_svgp":
                ...
                self.surrogate_manager = SurrogateManager(
                    n_objectives=self.n_objectives, model_type="gp", min_samples=self.min_samples
                )
                logger.info(f"[Trainer] Gaussian Process initialized")

            # 更新状态
            self.shared_memory.update_trainer_status(
                {"status": "ready", "n_samples": 0, "model_version": 0, "model_quality": None}
            )
            ...
            self.shared_memory.update_trainer_status(
                {"status": "error", "n_samples": 0, "model_version": 0, "model_quality": None, "error": str(e)}
            )
                ...
                if current_time - last_check_time >= poll_interval:
                    ...
                    if new_evals:
                        ...
                        if n_new >= self.min_new_samples_to_train:
                            logger.info(
                                f"[Trainer] Triggering training ({n_new} new samples >= {self.min_new_samples_to_train})"
                            )
                            self._train_with_new_data(new_evals)
                        else:
                            logger.info(
                                f"[Trainer] Waiting for more data ({n_new}/{self.min_new_samples_to_train} new samples)"
                            )
    ...
    def _train_with_new_data(self, new_evals: List[Dict]):
        ...
        try:
            ...
            if filtered_count > 0:
                logger.info(
                    f"[Trainer] Filtered {filtered_count} samples with abnormal objective values (e.g., S11=1000.0)"
                )
            ...
            for i in range(y.shape[1]):
                logger.info(
                    f"  Objective {i}: min={y[:, i].min():.4f}, max={y[:, i].max():.4f}, mean={y[:, i].mean():.4f}"
                )
            ...
            self.shared_memory.send_trainer_signal(
                "model_ready", {"version": self.current_version, "n_samples": n_samples, "quality": model_quality}
            )
