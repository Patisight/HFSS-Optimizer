...
from utils.formula import SParameterData as FormulaSData
...
class ObjectiveEvaluator:
    ...
    def _evaluate_formula(self, obj: Dict) -> Tuple[float, float]:
        ...
        if full_s_data is None:
            logger.info(
                f"[WARN] _evaluate_formula: failed to get S-parameter data for formula '{formula}' in objective '{obj.get('name', 'unknown')}'"
            )
            return 1000.0, 1000.0
...
def format_results(results: Dict[str, ObjectiveResult], objectives_config: List[Dict]) -> str:
    """
    格式化结果为字符串
