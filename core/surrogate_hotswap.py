...
from core.shared_memory import SharedMemoryManager, HotSwapManager
...
class SurrogateManagerWithHotSwap:
    ...
    def __init__(self, n_objectives: int, model_type: str = 'gp',
                 shared_dir: str = './shared_data',
                 min_samples: int = 5, n_estimators: int = 100,
                 **model_params):
        """
        初始化支持热替换的代理模型管理器
    ...
    def initialize_from_shared_memory(self) -> bool:
        ...
        try:
            # 恢复模型状态
            self.surrogate.surrogate.X_samples = model_state.get('X_samples', [])
            self.surrogate.surrogate.y_samples = model_state.get('y_samples', [])
    ...
    def check_and_swap(self) -> bool:
        ...
        with self.model_lock:
            ...
            try:
                ...
                # 备份当前模型（用于回滚）
                backup_X = self.surrogate.surrogate.X_samples
                backup_y = self.surrogate.surrogate.y_samples
                ...
                self.swap_history.append({
                    ...
                    'n_samples': len(self.surrogate.surrogate.X_samples),
                ...
                # 回滚
                self.surrogate.surrogate.X_samples = backup_X
                self.surrogate.surrogate.y_samples = backup_y
    ...
    def get_n_samples(self) -> int:
        """获取训练样本数"""
        return len(self.surrogate.surrogate.X_samples)
