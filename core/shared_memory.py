...
class SharedMemoryManager:
    ...
    def _init_files(self):
        ...
        if not os.path.exists(self.control_file):
            self._write_json(
                self.control_file,
                {"optimizer_signal": "ready", "trainer_signal": "ready", "timestamp": datetime.now().isoformat()},
            )
        ...
        if not os.path.exists(self.status_file):
            self._write_json(
                self.status_file,
                {
                    ...
                    "trainer": {"status": "idle", "n_samples": 0, "model_version": 0, "model_quality": None},
        ...
        if not os.path.exists(self.model_version_file):
            self._write_json(
                self.model_version_file, {"version": 0, "n_samples": 0, "timestamp": datetime.now().isoformat()}
            )
...
class DataWatcher:
    ...
    def __init__(self, shared_memory: SharedMemoryManager, callback=None, poll_interval: float = 1.0):
        """
        初始化数据监听器
