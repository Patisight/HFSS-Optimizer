...
• 算法参数"""
    ...
    def add_objective(self):
        ...
        if name and goal:
            ...
            self.obj_tree.insert('', tk.END, values=(name, type_names.get(obj_type, obj_type), freq, goal, display_target, weight))
    ...
    def update_objective(self):
        ...
        if selected:
            ...
            type_names = {'s_db': 'S参数(dB)', 'peak_gain': '峰值增益'}
            display_target = 'min' if target == 'minimize' else 'max'
            self.obj_tree.item(selected[0], values=(name, type_names.get(obj_type, obj_type), freq, goal, display_target, weight))
            self.update_objectives_data()
    ...
    def on_objective_select(self, event):
        if self.obj_tree.selection():
            ...
            self.obj_freq.delete(0, tk.END); self.obj_freq.insert(0, v[2])
...
推荐代理模型: GP (高斯过程) - 提供不确定性估计，指导下一步测哪里"""
        ...
        algo_combo = ttk.Combobox(algo_row, textvariable=self.algorithm_var, 
        ...
        ttk.Checkbutton(self.surrogate_frame, text="启用代理模型辅助", 
                        variable=self.use_surrogate_var).pack(anchor=tk.W, pady=2)
        ...
        surr_combo = ttk.Combobox(surr_row, textvariable=self.surrogate_var, 
                                   values=["gp", "rf", "incremental", "gpflow_svgp"], width=15, state="readonly")
        ...
        ttk.Spinbox(min_row, from_=3, to=20, textvariable=self.surrogate_min_samples_var, width=8).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(thresh_row, from_=0.1, to=2.0, increment=0.1, textvariable=self.surrogate_threshold_var, width=8, format="%.1f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Combobox(mobo_row, textvariable=self.acquisition_var, 
                     values=["ehvi", "ei", "ucb"], width=10, state="readonly").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(inertia_row, from_=0.2, to=0.9, increment=0.1, textvariable=self.inertia_var, width=8, format="%.1f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(c1_row, from_=0.5, to=3.0, increment=0.1, textvariable=self.c1_var, width=8, format="%.1f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(c2_row, from_=0.5, to=3.0, increment=0.1, textvariable=self.c2_var, width=8, format="%.1f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(mut_row, from_=0.05, to=0.5, increment=0.01, textvariable=self.mutation_var, width=8, format="%.2f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Spinbox(cross_row, from_=0.5, to=1.0, increment=0.05, textvariable=self.crossover_var, width=8, format="%.2f").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(nsga2_desc_row, text="⚠️ NSGA2 不使用代理模型，仿真次数 = 种群 × 迭代", foreground="orange").pack(anchor=tk.W)
        ...
        ttk.Spinbox(early_row2, from_=1, to=50, increment=1, textvariable=self.n_solutions_stop_var, width=8).pack(side=tk.LEFT, padx=5)
...
【代理模型】gp=高斯过程 | rf=随机森林 | incremental=RFF+SGD增量 | gpflow_svgp=稀疏变分GP【复杂场景推荐】"""
    ...
    def load_data_to_ui(self):
        ...
        for obj in self.config.get('objectives', []):
            ...
            self.obj_tree.insert('', tk.END, values=(obj.get('name', ''), type_names.get(obj['type'], obj['type']), freq_str, obj.get('goal', ''), display_target, weight))
        self.update_objectives_data()
