...
class OptimizationVisualizer:
    ...
    def update(
        ...
        pareto_params: np.ndarray = None,
        pareto_objectives: np.ndarray = None,
        surrogate_preds: np.ndarray = None,
        is_surrogate_prediction: bool = False,
    ):
        """
        更新数据并生成图表
        ...
        if is_surrogate_prediction:
            ...
            if surrogate_preds is not None:
                ...
                self.surrogate_comparison_data.append(
                    {"iteration": iteration, "real_objectives": obj_list, "pred_objectives": pred_list}
                )
            ...
            for idx, line in enumerate(f, 1):
                if line.strip():
                    try:
                        ...
                        self.evaluations.append({"iteration": idx, "params": param_list, "objectives": obj_list})
                        count += 1
                    except Exception:
                        continue
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                if is_dominated[j]:
                    continue
                # 检查 i 是否支配 j
                if np.all(objectives_matrix[i] <= objectives_matrix[j]) and np.any(
                    objectives_matrix[i] < objectives_matrix[j]
                ):
                    is_dominated[j] = True
                # 检查 j 是否支配 i
                elif np.all(objectives_matrix[j] <= objectives_matrix[i]) and np.any(
                    objectives_matrix[j] < objectives_matrix[i]
                ):
                    is_dominated[i] = True
                    break
        ...
        if self.n_objectives == 2:
            ...
            if len(surrogate_objs) > 0:
                ax.scatter(
                    ...
                    marker="x",
                    alpha=0.7,
                    label=f"Surrogate ({len(surrogate_objs)})",
                )
            ...
            if pareto_objectives is not None and len(pareto_objectives) > 0:
                ...
                ax.scatter(
                    ...
                    marker="^",
                    alpha=0.9,
                    label=f"Pareto Front ({len(pareto_display)})",
                    zorder=5,
                )
            ...
            surrogate_objs = (
                np.array(surrogate_objs_list) if len(surrogate_objs_list) > 0 else np.array([]).reshape(0, 3)
            )
            ...
            if pareto_objectives is not None and len(pareto_objectives) > 0:
                ax.scatter(
                    ...
                    marker="^",
                    alpha=0.9,
                    label=f"Pareto Front ({len(pareto_objectives)})",
                )
            ...
            for i, ax in enumerate(axes):
                ...
                for e in self.evaluations:
                    ...
                    if isinstance(obj, list) and len(obj) > i:
                        ...
                        if (
                            abs(abs(val) - PENALTY_VALUE) < 1e-6
                            or abs(val - SIMULATION_FAILURE_VALUE) < 1e-6
                            or abs(val) > ABNORMAL_THRESHOLD
                        ):
                            continue
                ...
                ax.set_xlabel("Iteration")
                obj_name = (
                    self.objectives[i].get("name", f"Objective {i+1}")
        ...
        try:
            pareto_objectives = np.array(
                [
                    (
                        s["objectives"]
                        if isinstance(s.get("objectives"), list)
                        else (
                            list(s["objectives"].values())
                            if isinstance(s.get("objectives"), dict)
                            else [s.get("objectives", 0)]
                        )
                    )
                    for s in pareto_solutions
                    if s.get("objectives") is not None
                ]
            )
            pareto_params = np.array(
                [s["parameters"] if "parameters" in s else s.get("params", []) for s in pareto_solutions]
            )
    ...
    def _generate_text_report(self, pareto_solutions: List[Dict], stats: Dict):
        ...
        def append_solution_lines(report_lines, sol, rank, marker=""):
                        ...
                        if goal is not None:
                            if target == "minimize":
                                ...
                                if display_val >= goal:
                                    ...
                                    obj_score = max(0, 100 - deficit_ratio * 40)
                            report_lines.append(
                                f"    {name}: {display_val:.4f} (goal: {goal}, score: {obj_score:.0f}/100)"
                            )
                        else:
                            report_lines.append(f"    {name}: {display_val:.4f}")
            ...
            var_names = (
                [v.get("name", f"Var{j}") for j, v in enumerate(self.variables)]
                if self.variables
                else [f"Var{j}" for j in range(len(params))]
            )
