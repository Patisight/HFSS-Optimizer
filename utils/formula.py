...
class Parser:
    ...
    def eat(self, token_type: TokenType):
        ...
        if self.current_token().type == token_type:
            ...
            raise FormulaSyntaxError(
                f"期望 {token_type}, 实际 {self.current_token().type} " f"at position {self.current_token().position}"
            )
    ...
    def parse(self) -> ASTNode:
        ...
        if self.current_token().type != TokenType.EOF:
            raise FormulaSyntaxError(f"解析后仍有剩余内容 at position {self.current_token().position}")
        return node
    ...
    def factor(self) -> ASTNode:
        ...
        if token.type == TokenType.NUMBER:
            ...
            raise FormulaSyntaxError(f"无法解析的词法单元 {token.type} at position {token.position}")
